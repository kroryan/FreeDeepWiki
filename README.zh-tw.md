...
![FreeDeepWiki 橫幅](screenshots/Freedeepwiki.png)

**FreeDeepWiki** 可以為任何 GitHub、GitLab 或 BitBucket 程式碼儲存庫自動建立美觀、互動式的 Wiki！只需輸入儲存庫名稱，FreeDeepWiki 將：
...
有關使用 FreeDeepWiki 搭配 Ollama 和 Docker 的詳細說明，請參閱 [Ollama 操作說明](Ollama-instruction.md)。
...
FreeDeepWiki 現在實作了靈活的基於提供商的模型選擇系統，支援多種 LLM 提供商：
...
# 設定檔目錄
FREEDEPWIKI_CONFIG_DIR=/path/to/custom/config/dir  # 可選，用於自訂設定檔位置
```

### 設定檔

FreeDeepWiki 使用 JSON 設定檔來管理系統的各個層面：
...
預設情況下，這些檔案位於 `api/config/` 目錄中。您可以使用 `FREEDEPWIKI_CONFIG_DIR` 環境變數自訂它們的位置。
...
**即將推出**：在未來的更新中，FreeDeepWiki 將支援一種模式，讓使用者需要在請求中提供自己的 API 金鑰。這將允許擁有私有通道的企業客戶使用其現有的 API 安排，而不必與 FreeDeepWiki 部署共享憑證。
...
FreeDeepWiki 使用 Python 的內建 `logging` 模組進行診斷輸出。您可以透過環境變數設定詳細程度和日誌檔案目標：
...
| `FREEDEPWIKI_AUTH_MODE` | 設定為 `true` 或 `1` 以啟用授權模式 | 否 | 預設為 `false`。如果啟用，則需要 `FREEDEPWIKI_AUTH_CODE` |
| `FREEDEPWIKI_AUTH_CODE` | 當 `FREEDEPWIKI_AUTH_MODE` 啟用時，Wiki 產生所需的秘密代碼 | 否 | 只有在 `FREEDEPWIKI_AUTH_MODE` 為 `true` 或 `1` 時才使用 |

如果您不使用 ollama 模式，您需要設定 OpenAI API 金鑰用於嵌入。其他 API 金鑰只有在設定並使用對應提供商的模型時才需要。

## 授權模式

FreeDeepWiki 可以設定為在授權模式下執行，在此模式下，Wiki 產生需要有效的授權代碼。如果您想控制誰可以使用產生功能，這會很有用。
限制前端啟動並保護快取刪除，但如果直接存取 API 端點，無法完全防止後端產生。

要啟用授權模式，請設定以下環境變數：

- `FREEDEPWIKI_AUTH_MODE`：將此設定為 `true` 或 `1`。啟用時，前端將顯示授權代碼的輸入欄位。
- `FREEDEPWIKI_AUTH_CODE`：將此設定為所需的秘密代碼。限制前端啟動並保護快取刪除，但如果直接存取 API 端點，無法完全防止後端產生。

如果未設定 `FREEDEPWIKI_AUTH_MODE` 或設定為 `false`（或除 `true`/`1` 以外的任何其他值），授權功能將被停用，不需要任何代碼。

### Docker 設定

您可以使用 Docker 來執行 FreeDeepWiki：
...
FreeDeepWiki 現在支援 [OpenRouter](https://openrouter.ai/) 作為模型提供商，讓您可以透過單一 API 存取數百個 AI 模型：
...
### 如何在 FreeDeepWiki 中使用 OpenRouter
...
[![FreeDeepWiki 展示影片](https://img.youtube.com/vi/zGANs8US8B4/0.jpg)](https://youtu.be/zGANs8US8B4)

*觀看 FreeDeepWiki 實際操作！*

## 🔧 配置選項

### 模型提供商

FreeDeepWiki 支援多個 AI 模型提供商：
...
[![Star 歷史圖表](https://api.star-history.com/svg?repos=AsyncFuncAI/freedeepwiki&type=Date)](https://star-history.com/#AsyncFuncAI/freedeepwiki&Date)
