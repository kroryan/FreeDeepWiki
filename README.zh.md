...
![FreeDeepWiki 横幅](screenshots/Freedeepwiki.png)

**FreeDeepWiki**可以为任何GitHub、GitLab或BitBucket代码仓库自动创建美观、交互式的Wiki！只需输入仓库名称，FreeDeepWiki将：
...
![FreeDeepWiki主界面](screenshots/Interface.png)
*FreeDeepWiki的主界面*
...
[![FreeDeepWiki演示视频](https://img.youtube.com/vi/zGANs8US8B4/0.jpg)](https://youtu.be/zGANs8US8B4)
...
[![星标历史图表](https://api.star-history.com/svg?repos=AsyncFuncAI/freedeepwiki&type=Date)](https://star-history.com/#AsyncFuncAI/freedeepwiki&Date)

## 🤖 基于提供者的模型选择系统

FreeDeepWiki 现在实现了灵活的基于提供者的模型选择系统，支持多种 LLM 提供商：
...
**即将推出**：在未来的更新中，FreeDeepWiki 将支持一种模式，用户需要在请求中提供自己的 API 密钥。这将允许拥有私有渠道的企业客户使用其现有的 API 安排，而不是与 FreeDeepWiki 部署共享凭据。
...
# 配置目录
FREEDEPWIKI_CONFIG_DIR=/path/to/custom/config/dir  # 可选，用于自定义配置文件位置

# 授权模式
FREEDEPWIKI_AUTH_MODE=true  # 设置为 true 或 1 以启用授权模式
FREEDEPWIKI_AUTH_CODE=your_secret_code # 当 FREEDEPWIKI_AUTH_MODE 启用时所需的授权码
```
如果不使用ollama模式，那么需要配置OpenAI API密钥用于embeddings。其他密钥只有配置并使用使用对应提供商的模型时才需要。

## 授权模式

FreeDeepWiki 可以配置为在授权模式下运行，在该模式下，生成 Wiki 需要有效的授权码。如果您想控制谁可以使用生成功能，这将非常有用。
限制使用前端页面生成wiki并保护已生成页面的缓存删除，但无法完全阻止直接访问 API 端点生成wiki。主要目的是为了保护管理员已生成的wiki页面，防止被访问者重新生成。

要启用授权模式，请设置以下环境变量：

- `FREEDEPWIKI_AUTH_MODE`: 将此设置为 `true` 或 `1`。启用后，前端将显示一个用于输入授权码的字段。
- `FREEDEPWIKI_AUTH_CODE`: 将此设置为所需的密钥。限制使用前端页面生成wiki并保护已生成页面的缓存删除，但无法完全阻止直接访问 API 端点生成wiki。

如果未设置 `FREEDEPWIKI_AUTH_MODE` 或将其设置为 `false`（或除 `true`/`1` 之外的任何其他值），则授权功能将被禁用，并且不需要任何代码。

### 配置文件

FreeDeepWiki使用JSON配置文件管理系统的各个方面：
...
默认情况下，这些文件位于`api/config/`目录中。您可以使用`FREEDEPWIKI_CONFIG_DIR`环境变量自定义它们的位置。
