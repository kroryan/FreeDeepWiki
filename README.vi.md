...
**Open FreeDeepWiki** là 1 triển khai thay thế cho FreeDeepWiki, tự động tạo ra các trang wiki cho bất kỳ Repository  nào trên GitHub, GitLab hoặc BitBucket! Chỉ cần nhập đường dẫn Repository, và FreeDeepWiki sẽ:
...
## 🔍 Cách Open Freedeepwiki hoạt động

FreeDeepWiki dùng AI để:
...
Bạn có thể dùng Docker để run FreeDeepWiki:
...
FreeDeepWiki hiện đã triển khai một hệ thống lựa chọn mô hình linh hoạt dựa trên nhiều nhà cung cấp LLM:
...
# Thư mục cấu hình
FREEDEPWIKI_CONFIG_DIR=/đường/dẫn/đến/thư_mục/cấu_hình  # Tùy chọn, cho vị trí tệp cấu hình tùy chỉnh
```

### Tệp cấu hình

FreeDeepWiki sử dụng các tệp cấu hình JSON để quản lý các khía cạnh khác nhau của hệ thống:
...
Mặc định, các tệp này nằm trong thư mục `api/config/`. Bạn có thể tùy chỉnh vị trí của chúng bằng biến môi trường `FREEDEPWIKI_CONFIG_DIR`.
...
**Sắp ra mắt**: Trong các bản cập nhật tương lai, FreeDeepWiki sẽ hỗ trợ chế độ mà người dùng cần cung cấp API key của riêng họ trong các yêu cầu. Điều này sẽ cho phép khách hàng doanh nghiệp có kênh riêng sử dụng cấu hình API hiện có mà không cần chia sẻ thông tin đăng nhập với triển khai FreeDeepWiki.

## 🔌 Tích hợp OpenRouter

FreeDeepWiki hiện đã hỗ trợ [OpenRouter](https://openrouter.ai/) làm nhà cung cấp mô hình, cho phép bạn truy cập hàng trăm mô hình AI thông qua một API duy nhất:
...
*Giao diện chính của FreeDeepWiki*
