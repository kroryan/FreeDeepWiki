...
**FreeDeepWiki**는 제가 직접 구현한 프로젝트로, GitHub, GitLab 또는 BitBucket 저장소에 대해 아름답고 대화형 위키를 자동 생성합니다! 저장소 이름만 입력하면 FreeDeepWiki가 다음을 수행합니다:
...
FreeDeepWiki는 AI를 사용하여 다음을 수행합니다:
...
FreeDeepWiki는 시스템의 다양한 측면을 관리하기 위해 JSON 설정 파일을 사용합니다:
...
기본적으로 이러한 파일은 `api/config/` 디렉토리에 위치합니다. `FREEDEPWIKI_CONFIG_DIR` 환경 변수를 사용하여 위치를 사용자 정의할 수 있습니다.

### Docker 설정

Docker를 사용하여 FreeDeepWiki를 실행할 수 있습니다:
...
FreeDeepWiki는 이제 여러 LLM 제공자를 지원하는 유연한 제공자 기반 모델 선택 시스템을 구현했습니다:
...
**출시 예정**: 향후 업데이트에서 FreeDeepWiki는 사용자가 요청에서 자신의 API 키를 제공해야 하는 모드를 지원할 예정입니다. 이를 통해 비공개 채널이 있는 기업 고객은 FreeDeepWiki 배포와 자격 증명을 공유하지 않고도 기존 API 구성을 사용할 수 있습니다.

## 🔌 OpenRouter 통합

FreeDeepWiki는 이제 [OpenRouter](https://openrouter.ai/)를 모델 제공자로 지원하여, 단일 API를 통해 수백 개의 AI 모델에 접근할 수 있습니다:
...
![FreeDeepWiki Main Interface](screenshots/Interface.png)
*FreeDeepWiki의 메인 인터페이스*
